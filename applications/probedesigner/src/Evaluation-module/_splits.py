...
def generate_evaluation_splits(
    ...
    """Generate stratified k-fold train/test splits for NMF and Tangram evaluation.
    ...
    The splits are always a cell-type-stratified ``StratifiedKFold``. A caller that
    wants a single held-out split takes fold 0.
    ...
    Returns:
        List of ``n_splits`` ``(train_idx, test_idx, per_celltype_splits)`` tuples.
    """
