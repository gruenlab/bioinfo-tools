#!/usr/bin/env python3
"""Standalone PCA variability evaluation using pre-normalised reconstruction.

Fits PCA on pre-normalised ``adata.X`` (log-normalised during preprocessing),
then compares reconstruction quality using only probe-gene expression.  The
component matrix H is fixed from the training fit; scores W are solved via
least squares from probe genes only.  No on-the-fly normalisation is applied;
``adata.X`` is used directly so that PCA and ridge results are comparable.

Usage::

    python run_pca_evaluation.py \\
    ...
    python run_pca_evaluation.py --dry_run \\
        --input_file preprocessed/full_transcriptome.h5ad \\
        --preprocessed_dir preprocessed/ \\
        --output_dir Evaluation-Results/
"""
...
from pca import pca_reconstruction, pca_reconstruction_by_celltype, _compute_full_pca_baseline
...
@dataclass
class PcaEvaluationConfig:
    """Runtime configuration for the PCA variability evaluation pipeline.
    ...
    Attributes:
        ...
        output_dir: Root output directory. Results go into
            ``Variability-Evaluation/results/PCA/``.
...
def run_pca_variability_evaluation(
    ...
    """Run PCA variability evaluation using pre-normalised ``adata.X``.

    Fits PCA on the pre-normalised training matrix (``adata.X``), reconstructs
    the full transcriptome from probe genes via least-squares solve with fixed
    component matrix H, then evaluates in the same pre-normalised space.
    ...
    Args:
        ...
        expvar_modes: Optional list of explained-variance modes to additionally report
            (see ``pca.py::pca_reconstruction``). Defaults to ``None``, i.e. ``[expvar_mode]``.
        gene_subsets: Optional list of gene subsets to score against. Defaults to
            ``None``, i.e. ``["all_genes"]``.

    Returns:
        Dict with ``"pca"`` (aggregated) and ``"pca_per_fold"`` DataFrames.

    Raises:
        ValueError: If ``adata.X`` appears to contain raw counts.
    """
    logger.info("=" * 80)
    logger.info("PCA VARIABILITY EVALUATION (%d fold(s))", len(splits))
    logger.info("=" * 80)

    if is_anndata_raw(adata_full):
        raise ValueError(
            "run_pca_variability_evaluation: adata_full.X appears to contain raw counts. "
            "PCA evaluation expects pre-normalised data in adata.X."
        )
    ...
    logger.info(
        "Evaluating PCA variability for %d panels across %d fold(s)...",
    ...
    for fold, (train_idx, test_idx, ct_splits_idx) in enumerate(splits):
        ...
        # Pre-compute PCA baseline for the default n_components, reusing the same
        # helper pca_reconstruction() falls back to on a cache miss (single
        # implementation — see pca.py::_compute_full_pca_baseline).
        cached_full_pca: dict[int, Any] = {
            n_components: _compute_full_pca_baseline(
                A_train, A_test, n_components, random_state,
            )
        }
        cached_ct_pca: dict[str, dict] = {}

        for panel_name in tqdm(panel_names, desc=f"Fold {fold + 1} PCA eval", leave=False):
            ...
            try:
                _evaluate_single_panel(
                    ...
                    cached_full_pca=cached_full_pca,
                    cached_ct_pca=cached_ct_pca,
                ...
                logger.warning(
                    "Fold %d: PCA evaluation failed for '%s': %s",
    ...
    if all_rows:
        ...
        per_fold_dir = results_dir / "PCA" / "per_fold"
        per_fold_dir.mkdir(parents=True, exist_ok=True)
        _save_results_per_dataset(df_all, per_fold_dir, "PCA (per-fold)")

        df_agg = _aggregate_fold_results(df_all)
        agg_dir = results_dir / "PCA"
        _save_results_per_dataset(df_agg, agg_dir, "PCA (aggregated)")

        results["pca"] = df_agg
        results["pca_per_fold"] = df_all

    logger.info("PCA variability evaluation complete. Skipped: %s", skipped or "none")
    return results
...
def _evaluate_single_panel(
    ...
    """Run PCA evaluation for one panel and append result rows in place.
    ...
    Args:
        ...
        cached_full_pca: Global PCA cache (mutated in place), keyed by
            ``n_components``.
        cached_ct_pca: Per-cell-type PCA cache (mutated in place).
        n_components: PCA components for this panel.
        celltype_col: obs column for cell-type labels.
        random_state: Random seed.
    ...
    """
    # ── Global PCA evaluation ─────────────────────────────────────────────────
    logger.info("  [%s] Global PCA (n=%d)...", panel_name, n_components)

    global_result = pca_reconstruction(
        adata=adata_full,
        probeset_genes=probeset_genes,
        A_train=A_train,
        A_test=A_test,
        ...
        random_state=random_state,
        cached_full_pca=cached_full_pca.get(n_components),
        expvar_mode=expvar_mode,
        expvar_modes=expvar_modes,
        gene_subsets=gene_subsets,
    )
    ...
    if global_result:
        if "computed_full_pca" in global_result:
            cached_full_pca[n_components] = global_result.pop("computed_full_pca")
        else:
            global_result.pop("computed_full_pca", None)
    ...
    # ── Per-cell-type PCA evaluation ──────────────────────────────────────────
    if celltype_col in adata_full.obs.columns and ct_splits_idx:
        logger.info("  [%s] Per-cell-type PCA...", panel_name)

        ct_result = pca_reconstruction_by_celltype(
            adata=adata_full,
            probeset_genes=probeset_genes,
            celltype_column=celltype_col,
            n_components=n_components,
            random_state=random_state,
            cached_full_pca_by_celltype=cached_ct_pca,
...
def _build_parser() -> argparse.ArgumentParser:
    """Build CLI argument parser for the PCA evaluation script."""
    p = argparse.ArgumentParser(
        description=(
            "PCA variability evaluation: fits PCA on log-normalised counts "
    ...
    p.add_argument(
        ...
        help="Root output directory. Results go into Variability-Evaluation/results/PCA/.",
    )
...
def _args_to_config(args: argparse.Namespace) -> PcaEvaluationConfig:
    return PcaEvaluationConfig(
...
def _save_parameters(config: PcaEvaluationConfig) -> None:
    config.output_dir.mkdir(parents=True, exist_ok=True)
    params = asdict(config)
    params["timestamp"] = datetime.now().isoformat()
    params["script"] = "run_pca_evaluation.py"
    param_file = config.output_dir / "pca_evaluation_parameters.json"
...
def main() -> None:
    """Entry point: parse CLI arguments and run the PCA evaluation."""
    ...
    log_path = (
        config.output_dir
        / "logs"
        / f"pca_evaluation_{datetime.now():%Y%m%d_%H%M%S}.log"
    )
    ...
    logger.info("=" * 80)
    logger.info("PCA VARIABILITY EVALUATION PIPELINE")
    logger.info("=" * 80)
    _log_memory("start")
    ...
    if is_anndata_raw(adata_full):
        logger.error(
            ...
            "The PCA evaluation requires pre-normalised data in adata.X.",
            ref_path,
        )
        sys.exit(1)
    ...
    # ── Run PCA variability evaluation ───────────────────────────────────────
    run_pca_variability_evaluation(
        adata_full=adata_full,
    ...
    _log_memory("after PCA variability evaluation")

    logger.info("=" * 80)
    logger.info("PCA PIPELINE COMPLETE – outputs in: %s", config.output_dir)
    logger.info("=" * 80)
